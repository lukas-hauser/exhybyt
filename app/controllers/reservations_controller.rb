...
class ReservationsController < ApplicationController
  before_action :logged_in_user, except: %i[current_exhibitions past_exhibitions upcoming_exhibitions]
  before_action :active_space,          only: [:create]
  before_action :stripe_ready,          only: [:create]
  before_action :approved_reservations, only: %i[current_exhibitions past_exhibitions upcoming_exhibitions]
  before_action :set_reservation,       only: %i[success cancel]
  ...
  def own_space
    @space = Space.find(params[:space_id])
    return unless current_user?(@space.user)
